...
public class CategoryServiceImpl extends ServiceImpl<CategoryMapper,Categories> implements CategoryService {
    ...
    @Override
    @CacheResult(key = "0", prefix = CATEGORY_KEY)
...
//                ));
    ...
    @Override
    @CacheResult(prefix = CACHE_POST_COUNT_KEY,key="0")
    public List<Map<Long, Integer>> countWithCategory() {
        // MP 分组查询：按 category_id 分组统计数量
        ...
        // 转换成接口需要的格式 List<Map<Long, Integer>>
        return list.stream().map(map -> {
            Map<Long, Integer> resultMap = new HashMap<>();
            Long categoryId = Long.valueOf(map.get("category_id").toString());
            Integer count = Integer.valueOf(map.get("count").toString());
            resultMap.put(categoryId, count);
            return resultMap;
        }).collect(Collectors.toList());
    }
